define("com/clorox/DOSTablet/userDOSTabletController", function() {
    ...
    return {
        ...
        setFormData: function(doadosData) {
            try {
                ...
                if (!Utils.isNullorEmpty(gblDosDataArray)) {
        ...
        setSegmentsData: function() {
            try {
                ...
                if (!Utils.isNullorEmpty(data)) {
                    ...
                    if (!Utils.isNullorEmpty(data.Common)) this.view.DOSHeader.setHeaderData(data.Common[0], dosDataBackHead[k]);
                    if (!Utils.isNullorEmpty(data.metric)) this.setOverViewData(data.metric, data.Common[0]);
                }
        ...
        setOverViewData: function(metrics, commonData) {
            try {
                if (!Utils.isNullorEmpty(metrics) && metrics.length !== 0) {
                    ...
                    var dataToPush = [],
                        dataLeftOverview, dataRightOverview;
        ...
        getSegmentData: function(index, newMetrics, commonData) {
            try {
                ...
                var commonSkins = {
                    ...
                    adoCount: "sknLblFFFFFSSPR16px100"
                };
                var sortOrder = 4;
                var grpName = newMetrics[0];
                ...
                for (var j in metricsData) {
                    ...
                    if (!Utils.isNullorEmpty(metricsData[j].color_group)) {
                        if (metricsData[j].color_group === AppConstants.COLOR_GROUP_ORDERS) groupSkin = ordersGroupName;
                        else if (metricsData[j].color_group === AppConstants.COLOR_GROUP_SHIPMENTS) groupSkin = shipmentsGroupName;
                        else if (metricsData[j].color_group === AppConstants.COLOR_GROUP_FORECAST) groupSkin = forecastGroupName;
                        else groupSkin = ordersGroupName;
                    }
                    var finalValue = 0;
                    var finalCount = 0;
                    ...
                    if (groupId === AppConstants.GROUPVALUE) { //Common
                        segData["lblGroupName" + index] = {
                            text: grpName,
                            skin: groupSkin
                        };
                        segData["lblGroupValue" + index] = {
                            text: finalCount
                        };
                        segData["flxGroupMain" + index] = {
                            onClick: this.goToDosChart.bind(this, grpName)
                        };
                    }
                    if (grpName === AppConstants.GROUP_NAME_ADO) { //ADO
                        if (groupId === AppConstants.RIGHTVALUE) {
                            segData["lbIndexValue" + index] = {
                                text: name + ": "
                            };
                            segData["lblIndexCount" + index] = {
                                text: finalCount,
                                skin: commonSkins.adoCount
                            };
                        } else if (groupId === AppConstants.LEFTVALUE) {
                            segData["lblChangeValue" + index] = {
                                text: name + ": "
                            };
                            segData["lblChangeCount" + index] = {
                                text: finalCount,
                                skin: commonSkins.adoCount
                            };
                        }
                    } else if (grpName === AppConstants.GROUP_NAME_MTD_SHIPMENTS) { //SHIPMENTS
                        var mtdShipVal = (finalValue).toFixed(0);
                        if (groupId === AppConstants.RIGHTVALUE) {
                            var timeElapsed = commonData.timeelapsed.split("%", 1);
                            skinValue = (!Utils.isNullorEmpty(timeElapsed) && timeElapsed.length > 0 && mtdShipVal >= kony.os.toNumber(timeElapsed[0])) ? commonSkins.positiveCount : commonSkins.negativeCount;
                            ...
                            skinValue = (finalValue >= 0) ? commonSkins.positiveCount : commonSkins.negativeCount;
                        ...
                        var mtdOdrVal = (finalValue).toFixed(0);
                        if (groupId === AppConstants.RIGHTVALUE) {
                            skinValue = (finalValue > 100) ? commonSkins.positiveCount : commonSkins.negativeCount;
                            segData["lbIndexValue" + index] = {
                                text: name + ": "
                            };
                            segData["lblIndexCount" + index] = {
                            ...
                            skinValue = (finalValue >= 0) ? commonSkins.positiveCount : commonSkins.negativeCount;
                            if (grpName === AppConstants.GROUP_NAME_NDF) skinValue = (finalValue > 100) ? commonSkins.positiveCount : commonSkins.negativeCount;
                            segData["lblChangeValue" + index] = {
                                text: name + ": "
                            };
                            segData["lblChangeCount" + index] = {
        ...
        setAdditionalMetricsData: function(additionalMetrics) {
            try {
                ...
                if (s === addOrderMetrics.length && addOrderMetrics.length < addDataMetrics.length) {
                    for (var z = k; z < addDataMetrics.length; z++) {
                        dataLeft = {
                            flxLeft1: {
                                isVisible: false
                            }
        ...
        getSegData: function(index, productObj) {
            try {
                ...
                var finalVal = 0;
                var skinVal = "";
                if (productObj.color_group === AppConstants.COLOR_GROUP_ORDERS) skinVal = "sknLblA7DCECRB16px114";
                else if (productObj.color_group === AppConstants.COLOR_GROUP_SHIPMENTS) skinVal = "sknLblEEB64FRB16px114";
                else if (productObj.color_group === AppConstants.COLOR_GROUP_FORECAST) skinVal = "sknLblC597FFRB16px114";
                var kpiName = productObj.kpi_name;
                if (!Utils.isNullorEmpty(productObj.value)) {
                    finalVal = kony.os.toNumber(productObj.value);
                }
                if (kpiName == AppConstants.AVG_DAILY_ORDERS_VS_YA_INDEX) {
                    ...
                    data["lblCount" + index] = {
                        text: ((finalVal).toFixed(0)) && finalVal !== "None" ? parseInt(finalVal).toLocaleString('en-US') : "",
                        skin: (finalVal).toFixed(0) < 100 ? "sknLbl0AC7C2SSPR16px100" : "sknLblE34547SSPR16px100"
                    };
            ...
            try {
                ...
                if (chartDataMap.has(gblSelectedOverviewType) && chartDataMap.get(gblSelectedOverviewType).has(header)) {
                    ...
                    if (!Utils.isNullorEmpty(query)) {
                        GlobalServices.getOverview({
                            "query": query
                        }, DOS.serviceName, requestMethod, null, this.getOverviewAPISuccessCallBack, this.getOverviewAPIFailureCallBack);
                    }
                }
            } catch (e) {
...
define("com/clorox/DOSTablet/DOSTabletControllerActions", {
    /*
          This is an auto generated file and any modifications to it may result in corruption of the action sequence.
        */
});
define("com/clorox/DOSTablet/DOSTabletController", ["com/clorox/DOSTablet/userDOSTabletController", "com/clorox/DOSTablet/DOSTabletControllerActions"], function() {
    var controller = require("com/clorox/DOSTablet/userDOSTabletController");
    var actions = require("com/clorox/DOSTablet/DOSTabletControllerActions");
