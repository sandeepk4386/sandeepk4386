...
//       this._linesData = [];
      ...
      var urlConf = {URL: "linechart/index.html", requestMethod:constants.BROWSER_REQUEST_METHOD_GET};
    ...
    /**
         ...
         * @description: generates line chart by taking the data and the other params as input
      	 */
    createChart: function(dataSet, linesData) {
      ...
      try {
        ...
        if(this.validateAllParams(this._chartTitle, series,lines, this._chartProperties)) {
          ...
          for(var i =1; i < noOfSeries ; i++){
            seriesData += '{index:'+parseInt(lines[7][i-1])+',pointWidth: '+ this._chartProperties._pointWidth +',name: '+JSON.stringify(lines[0][i-1])+',data: '+JSON.stringify(series[i]) +
              ',showInLegend: true,color:'+JSON.stringify(lines[1][i-1])+',dashStyle:'+JSON.stringify(lines[6][i-1])+',type:'+JSON.stringify(lines[5][i-1]) +
              ',marker:{symbol:'+JSON.stringify(lines[2][i-1])+',fillColor:'+JSON.stringify(lines[3][i-1])+',lineWidth:'+parseInt(lines[4][i-1])+' ,enabled: '+false+',lineColor:null}},';
          }

          this._chartProperties._xAxisTickPositions = "[" + gblGraphXTickPositions + "]";

          var highChartData = '{chart:{panning:{enabled:'+true+',type:"xy"}, pinchType: "xy", zoomType: "xy", height:' + this._chartHeight + ',marginBottom:'+ this._chartProperties._bottomMargin +',marginRight:' + this._chartProperties._rightMargin + 
              ',marginLeft:' + this._chartProperties._leftMargin + ',style:{fontFamily:"Roboto"}},title: {' +
              'text:' + JSON.stringify(this._chartTitle) + ', align:"left" }, scrollbar: {enabled: false},' +
              'yAxis: {tickAmount:' + JSON.stringify(this._chartProperties._yAxisTickAmount) +
              ',endOnTick: ' + false + ',opposite: ' + true + ',gridLineColor: ' + JSON.stringify(this._chartProperties._gridLineColor) +
              ', title: {text:' + JSON.stringify(this._chartProperties._yAxisTitle) + '},labels:{formatter: '+ this._chartProperties._yAxisLabelsFormatter + 
              ', align:' + JSON.stringify(this._chartProperties._yAxisLabelAlign) + ',x:' + this._chartProperties._labelX + ',y:' + this._chartProperties._labelY + 
              ',style:{color: ' + JSON.stringify(this._chartProperties._yAxisLabelColor) + '}}},' +
              'xAxis: {categories: ' + JSON.stringify(series[0]) + ',lineColor: ' + JSON.stringify(this._chartProperties._xAxisLineColor) +
              ',tickWidth: ' + JSON.stringify(this._chartProperties._xAxisTickWidth) + ',tickColor: ' + JSON.stringify(this._chartProperties._xAxisTickColor) +
              ',tickPositions: ' + this._chartProperties._xAxisTickPositions + ',labels:{style:{color: ' + JSON.stringify(this._chartProperties._xAxisLabelColor) + ', fontFamily:"Roboto"}}},' +
              ...
              'plotOptions:{series: {states: {inactive:{opacity:'+this._chartProperties._opacity+'}, hover: { enabled: '+false+'}}}},'+
    ...
    validateAllParams: function(title, series, lines, properties) {
      ...
      try {
        ...
        if(!this.validateData(series, 'array')) {
          throw {"Error": "Invalid Datatype", "message": "Wrong datatype for series " + JSON.stringify(series1)};
        }       
      }
      catch(exception) {
        kony.print(JSON.stringify(exception));
    ...
    showGridChart: function() {
      try {
        ...
        if(!Utils.isNullorEmpty(this._chartData) && !Utils.isEmptyArray(this._chartData.data)) {
          this.view.brwLineChart.setVisibility(true);
          this.createChart(this._chartData,this._linesData);
        } else {
          this.view.brwLineChart.setVisibility(false);
          kony.print("Error: NoData  message: No data in data grid");
        }
      }
      catch(exception) {
        if(exception.Error === "NoData") {
