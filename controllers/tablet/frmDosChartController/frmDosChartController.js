  ...
  orientationChange : function(){
    try{
      if(kony.os.getDeviceCurrentOrientation() === constants.DEVICE_ORIENTATION_LANDSCAPE){
        ...
        this.view.flxScrlDos.setContentOffset({x:"0%",y:"0%"}, true);
    ...
    try{
      ...
      if(orientationChange && orientationChange === true){
        if(!Utils.isNullorEmpty(query)){
          GlobalServices.getOverview({"query":query}, DOS.serviceName, requestMethod, null, this.fetchChartDataSuccessCallBack.bind(this, orientationChange), this.fetchChartDataFailureCallBack);
        } 
      }else{
  ...
  businessOnClick : function(name,query,rowIndex){
    try{
      ...
      if(!dataAvailable){
        ...
        if(!Utils.isNullorEmpty(query)){
          GlobalServices.getOverview({"query":query}, DOS.serviceName, requestMethod, null, this.getOverviewDataSuccess, this.getOverviewDataFailure);
        }
      }
    }catch(e){
      Utils.hideLoadingIndicator();
  ...
  getOverviewDataSuccess : function(response){
    try{
      if(!Utils.isNullorEmpty(response) && !Utils.isNullorEmpty(response.records[0]) && !Utils.isNullorEmpty(response.records[0].Common)){
        ...
        var requestMethod = Utils.getRequestMethod(gblSelectedOverviewType, true);
        if(!Utils.isNullorEmpty(query)){
          GlobalServices.getOverview({"query":query}, DOS.serviceName, requestMethod, null, this.fetchGraphDataSuccessCallBack, this.getOverviewDataFailure);
        }
      }else{
        Utils.hideLoadingIndicator();
