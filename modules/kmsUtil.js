...
//               kmsUtil.incrementNotificationCount();
  ...
  unsubscribeMFMessaging: function()
  {
    try{
      kony.print("\n\n<----------in unsubscribeKMS-------->\n\n");
      KNYMobileFabric.getMessagingService().unregister(
        function(response){
  ...
  getFetchMessagesSuccessCallback: function(type,response){
    try{
      ...
      var pushTitle = "",pushMsg,pushType = "",posterImage,pushId="",pushDate,pushRichContent,found=[];
      var isRead = false;
      var msgType = "UNREAD";
      var lblNotifDate;
      var lblNotifMessage = {text:pushTitle,skin:"sknLblWhiteBold16Px"};
      if(!Utils.isNullorEmpty(fetchMessage) && fetchMessage !== []){
        ...
        for(var i=0;i<fetchMessage.length;i++){
          ...
          formatted_date = pushDate ? (new Date(pushDate).toLocaleString('en-us',{timezone: userTimeZone,month:'long', year:'numeric', day:'numeric'})) : "";
          time=pushDate ? (new Date(pushDate).toLocaleString('en-us',{timezone: userTimeZone,hour: 'numeric', minute:'numeric'})) : "";
          ...
          if(pushStatus === "Opened"){
            ...
            lblNotifMessage = {text:pushTitle,skin:"sknLblSSPR0f5ae94A3B3"};
            if(pushType === "dos_data_updated"){
              pushIcon = {text:"k",skin:"sknLblNotifyHoronIconRead"};
            }else if(pushType === "mtd_orders_vs_ya_index_exceeds" || pushType === "mtd_shipments_vs_ndf_exceeds"){
              pushIcon = {text:"c",skin:"sknLblNotifyBellWhiteRead"};
            }else{
              pushIcon = {text:"k",skin:"sknLblNotifyHoronIconRead"};
            }
            ...
            msgType = "UNREAD";
            lblNotifDate=lblNotifDate+" • "+msgType;
            lblNotifMessage = {text:pushTitle,skin:"sknLblWhiteBold16Px"};
            //kony.print("inside getFetchMessagesSuccessCallback unread message count : " + unreadCount);
            if(pushType === "dos_data_updated"){
              pushIcon = {text:"k",skin:"sknLblNotifyHoronIcon"};
            }else if(pushType === "mtd_orders_vs_ya_index_exceeds" || pushType === "mtd_shipments_vs_ndf_exceeds"){
              pushIcon = {text:"c",skin:"sknLblNotifyBellWhite"};
            }else{
              pushIcon = {text:"k",skin:"sknLblNotifyHoronIcon"};
            }
          }
