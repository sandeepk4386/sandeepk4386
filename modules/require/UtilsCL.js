define(function() {
  ...
  var Utils =  {
    ...
    navigate : function(frmName,params,formArray){
      ...
      if(!Utils.isNullorEmpty(currentForm) && currentForm.id!=frmName){
        if(formArray == G_Navigation_History){  
          if(formArray[formArray.length-1]!==currentForm.id)
      ...
      if (!kony.net.isNetworkAvailable(constants.NETWORK_TYPE_ANY)) { 
        ...
        if(!Utils.isNullorEmpty(currForm) && !Utils.isNullorEmpty(currForm.flxGenericError)){
            ...
            if(currForm.id === "frmDOSHome"){
              currForm.GenericError.displayText(kony.i18n.getLocalizedString("i18n.clorox.information"), message,
                                                kony.i18n.getLocalizedString("i18.clorox.ok"), Utils.dismissDOSInternetLossPopup);
            } else{
              currForm.GenericError.displayText(kony.i18n.getLocalizedString("i18n.clorox.information"), message,
                                                kony.i18n.getLocalizedString("i18.clorox.ok"), Utils.dismissInternetLossPopup);
            }
    ...
    showDefaultLoading : function(){
      kony.application.showLoadingScreen("sknFlxLoading",kony.i18n.getLocalizedString("i18n.clorox.loadingInd"),
    ...
    showErrorAlert : function (msg,title,handle){
      ...
      if(title == kony.i18n.getLocalizedString("i18n.clorox.timeOut"))
        yeslabel=kony.i18n.getLocalizedString("i18n.clorox.SignInTimeOut");
  ...
  */
    ...
    openWebURLForm : function(urlParams){
      if(urlParams){
        if(deviceUtil.isAndroid()){
          ...
          var paramObj = {urlParams : urlParams,formName:"frmRegistration"};
    ...
    openWebURL: function(urlParams){
      if(urlParams){
        var paramObj = {urlParams : urlParams,formName:"frmWebRegistration"};
        var navObj = new kony.mvc.Navigation("frmWebURL");
        navObj.navigate(paramObj);
      }
    },
    ...
    format : function(val, decimal,locale){
      try{
        ...
        }else if(!Utils.isNullorEmpty(num)){
    ...
    getCountryCdName: function(countyCode){
      if (G_SUPPORT_EMAILS.filter(function(e) { return e.countryCode === countyCode; }).length === 0) {
        return "";
      }else{
        // get country Name from array
        let thisFeature =G_SUPPORT_EMAILS.filter(function(e) { return e.countryCode === countyCode; })[0];
...
*/
    ...
    dumpError: function(inError, inShowObject)
    {
      ...
      var sObject = "";
      if (!Utils.isNullorEmpty(inError.allResponse) && inError.allResponse.length > 0 &&
          !Utils.isNullorEmpty(inError.allResponse[0].volumeDetailPeriods) &&
          inError.allResponse[0].volumeDetailPeriods.length > 0)
        sObject = inError.allResponse[0].volumeDetailPeriods[0];
    ...
    {
      var rtnBool = false;
      if (inString.trim().toLowerCase() === "true")
        rtnBool = true;
      return rtnBool;
    },
    ...
    encryptAESVA: function (text, key){
      ...
      encryptedTable = kony.crypto.encrypt("aes",
                                           key,
                                           text,
                                           { padding : paddingScheme, mode : "cbc", initializationvector : propertiesTable } );
    ...
    initLocaleVariables:function() {
      ...
      for(var i = 0; i<12; i++) {
        MONTHS_LONG.push(new Date(2020, i, 1).toLocaleString(currLocale, {month: "long"}).replace(/(^\w|\s\w)/g, m => m.toUpperCase()));
        MONTHS_SHORT.push(new Date(2020, i, 1).toLocaleString(currLocale, {month: "short"}).replace(/(^\w|\s\w)/g, m => m.toUpperCase()));
        if(i>=7) continue;
        DAY_NAMES.push(baseDate.toLocaleDateString(currLocale, { weekday: 'short' }).replace(/(^\w|\s\w)/g, m => m.toUpperCase()));
    ...
    getRequestMethod : function(overviewType, isChart = false){
      ...
      if(overviewType === kony.i18n.getLocalizedString("i18.clorox.mtdOrders") ){
        if(isChart === true)
          requestMethod = DOS.fetchMTDOrdersChart;
        else
          requestMethod = DOS.fetchMTDOrders;

      }else if( overviewType === kony.i18n.getLocalizedString("i18.clorox.avgDaily")){
        if(isChart === true)
          requestMethod =  DOS.fetchAvgDailyOrdersChart;
        else
          requestMethod =  DOS.fetchAvgDailyOrders;

      }else if(overviewType === kony.i18n.getLocalizedString("i18.clorox.mtdShipments")){
        if(isChart === true)
          requestMethod =  DOS.fetchMTDShipmentsChart;
        else
          requestMethod =  DOS.fetchMTDShipments;
      }else {
    ...
    feedbackOrientationChange: function(){
      try{
        var currForm = kony.application.getCurrentForm();
        if(deviceUtil.isTablet()){
          if(kony.os.getDeviceCurrentOrientation() === constants.DEVICE_ORIENTATION_LANDSCAPE){
            if(deviceUtil.isiPad()){
              currForm.Feedback.right= "-10dp";
            }else{
              currForm.Feedback.right= "-13dp";
            }
          }else{
            currForm.Feedback.right= "0dp";
          }
          currForm.Feedback.height="175dp";
          currForm.Feedback.width="5%";
        }
      }catch(e){
        kony.print("Exception in  feedbackOrientationChange :"+JSON.stringify(e));
      } 
    },
  };
  return Utils;
