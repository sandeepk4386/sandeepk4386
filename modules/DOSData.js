...
function fetchDOSData(formName,query,isNotification){
  ...
  GlobalServices.getOverview({"query":query}, DOS.serviceName, DOS.getOverview, null, getOverviewDataSuccess.bind(this,isNotification), getOverviewDataFailure);
}
    ...
    if(!Utils.isNullorEmpty(response) && !Utils.isNullorEmpty(response.records[0]) ){
      if(gblFormName === "frmDOSHome"){
        ...
        if(!Utils.isNullorEmpty(query)){
          GlobalServices.getOverview({"query":query}, DOS.serviceName, requestMethod, null, fetchChartDataSuccessCallBack, fetchChartDataFailureCallBack);}
      }
      Utils.hideLoadingIndicator();
    }else{
      Utils.hideLoadingIndicator();
...
function getAccurateValue(value){
  let splittedValue, finalValue ;
  if(value.endsWith('K') || value.endsWith('k') ){
    let localValue = value.toUpperCase();
    splittedValue = localValue.split('K' , 1)[0] ;
    finalValue = kony.os.toNumber(splittedValue) * 1000;
  } else if(value.endsWith('M') || value.endsWith('m')){
    let localValue = value.toUpperCase();
    splittedValue = localValue.split('M' , 1)[0] ;
    finalValue = kony.os.toNumber(splittedValue) * 1000000;
  }else{
